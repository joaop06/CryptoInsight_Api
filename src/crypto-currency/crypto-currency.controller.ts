...
import {
  ...
  FindOneDoc,
  UpdateDoc,
} from '../crypto-currency/dto/crypto-currency.documentation';
import {
...
export class CryptoCurrencyController
  implements CryptoCurrencyControllerInterface
{
  ...
  async create(
    @Body() object: CreateCryptoCurrencyDto,
  ...
  async update(
    @Param('id') id: string,
    @Body() object: Partial<CryptoCurrencyEntity>,
  ): Promise<any> {
    try {
  ...
  async findAll(
    @Query() options: FindOptionsDto<CryptoCurrencyEntity>,
  ): Promise<FindReturnModelDto<CryptoCurrencyEntity>> {
    try {
