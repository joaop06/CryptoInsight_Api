...
import {
  ...
  FindOneDoc,
  UpdateDoc,
} from './dto/investments.documentation';
import {
...
export class InvestmentsController implements InvestmentsControllerInterface {
  ...
  async create(
    @Body() object: CreateInvestmentDto,
  ...
  async update(
    @Param('id') id: string,
    @Body() object: Partial<InvestmentsEntity>,
  ): Promise<any> {
    try {
  ...
  async findAll(
    @Query() options: FindOptionsDto<InvestmentsEntity>,
  ): Promise<FindReturnModelDto<InvestmentsEntity>> {
    try {
