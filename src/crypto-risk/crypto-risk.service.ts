...
export class CryptoRiskService implements OnModuleInit {
  ...
  async classifyRisk(cryptos: CryptoCurrencyEntity[]): Promise<void> {
    ...
    await Promise.all(
      cryptos.map(async (crypto, i) => {
        try {
          if (
            !crypto.risk ||
            process.env.ENABLE_RISK_MODEL_TRAINING === 'true'
          ) {
            const risk = await this.getRiskClassification(crypto);
            await this.cryptoCurrencyService.update(crypto.id, {
              ...crypto,
              risk,
            });
  ...
  async getRiskClassification(crypto: CryptoCurrencyEntity): Promise<string> {
    try {
      ...
      if (!model) {
        const errorMessage =
          'Modelo de Classificação de Risco ainda não está treinado';
